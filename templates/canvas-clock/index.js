...
Page({
  ...
  canvasClock: function () {
    ...
    var R = width / 2 - 42;//设置文字距离时钟中心点距离  
    //重置画布函数  
    function reSet() {
      context.height = context.height;//每次清除画布，然后变化后的时间补上  
      context.translate(width / 3, height / 3);//设置坐标轴原点  
    ...
    function num() {
      ...
      for (var i = 1; i < 13; i++) {
        ...
        if (i == 11 || i == 12) {//调整数字11和12的位置  
          context.fillText(i, x - 5, y + 4);
        } else if (i == 10){//调整数字10的位置
          context.fillText(i, x - 4, y + 3);
        }
        else {
          context.fillText(i, x -2.5, y + 3);
        }
      }
    }
    //绘制小格  
    ...
    function move() {
      ...
      context.moveTo(-8, 0);//指针开始位置
      context.lineTo(width / 4 - 20, 0);//指针结束位置，可以决定指针长度
      context.stroke();
